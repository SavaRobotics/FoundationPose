...
class PoseTransformer(Processor):
    ...
    def process(self, data):
        ...
        if data.pose_matrix is None:
            data.add_error("PoseTransformer", "No pose matrix available")
            return data
            
        print("🔄 Transforming 4x4 pose matrix to 6D pose...")
        
        # Convert the pose matrix to 6D pose
        pose_6d = self._convert_pose_matrix_to_6d(data.pose_matrix)
